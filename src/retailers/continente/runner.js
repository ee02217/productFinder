...
const { fetchPage, delay, closeBrowser } = require('./http');
const { discoverAllProductUrls, isRootBlocked, BLOCKED_ROOT_CATEGORIES } = require('./discovery');
const { parseCategoryFromUrl, extractProductDataFromPage, toCents } = require('./parser');
...
function isUrlBlocked(url, blockedRules) {
  ...
  for (const rule of blockedRules) {
    ...
    if (!hasCat) continue;
    if (!rule.subcategorySlug) return true;
...
async function runWithJob(job, options) {
  ...
  console.log(`[CONTINENTE] Starting scrape: limit=${opts.limit}, delayMs=${opts.delayMs}`);
  console.log(`[CONTINENTE] Blocked root categories: ${BLOCKED_ROOT_CATEGORIES.join(', ')}`);

  let urls = [];
  try {
    console.log(`[CONTINENTE] Discovering categories and product URLs...`);
    urls = await discoverAllProductUrls({
      limit: opts.limit,
      delayMs: opts.delayMs,
    });
    console.log(`[CONTINENTE] Discovered ${urls.length} total product URLs`);
  } catch (err) {
    await markFinished(job.id, 'failed', { errors: job.errors + 1 });
    throw err;
  }

  // Get category blocks from DB (fine-grained blocking)
  ...
  // Fast pre-filter by URL path slugs
  const blockedRules = blocks.map(b => ({
    categorySlug: slugify(b.category),
    subcategorySlug: b.subcategory ? slugify(b.subcategory) : null,
  }));

  // Filter URLs: remove blocked root categories + blocked DB rules
  const discoveredUrls = urls.length;
  const allowedUrls = urls.filter((u) => {
    // First check root category blocking
    if (isRootBlocked(u.categorySlug)) return false;
    // Then check DB fine-grained blocking
    if (isUrlBlocked(u.url, blockedRules)) return false;
    return true;
  });
  ...
  console.log(`[CONTINENTE] discovered=${discoveredUrls}, skippedByBlock=${skippedByBlock}, selected=${totalUrls}`);
  ...
  for (let idx = cursor; idx < totalUrls; idx++) {
    ...
    try {
      ...
      const parsed = await extractProductDataFromPage(page, categorySlug);
      await page.close();

      // Add URL and category to parsed data
      parsed.url = url;
      parsed.category = item.category;

      // Convert price strings to cents
      if (parsed.price) {
        parsed.priceCents = toCents(parsed.price);
      }
      if (parsed.pricePerKg) {
        parsed.pricePerKgCents = toCents(parsed.pricePerKg);
      }
      if (parsed.pvp) {
        parsed.pvpCents = toCents(parsed.pvp);
      }

      stats.processed++;

      // Skip blocked categories/subcategories (defense in depth)
...
async function startBackground(options = {}) {
  if (runtime.running) {
    throw new Error('Continente scraper already running');
  }
  ...
  runtime.promise = runWithJob(job, opts)
    .catch(async (err) => {
      console.error(`[CONTINENTE] Error: ${err.message}`);
      await markFinished(job.id, 'failed', { errors: job.errors + 1 });
    })
    .finally(() => {
...
async function resumeBackground(jobId, options = {}) {
  if (runtime.running) throw new Error('Continente scraper already running');
