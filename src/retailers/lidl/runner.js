...
async function runWithJob(job, options) {
  ...
  console.log(`[LIDL] discovered=${discoveredUrls}, skippedByBlock=${skippedByBlock}, selected=${totalUrls}`);
...
async function startBackground(options = {}) {
  if (runtime.running) {
    throw new Error('Lidl scraper already running');
  }
...
async function resumeBackground(jobId, options = {}) {
  if (runtime.running) throw new Error('Lidl scraper already running');
